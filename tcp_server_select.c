...
int handle_client(int client_socket) {
    ...
    if (bytes_received < 0) {
        ...
        if (strerror_r(errno, ebuf, sizeof(ebuf)) == ERANGE) {
            ...
            printf("recv from client failed %s %d\n", ebuf,client_socket);
        }
        return 0;
    }
    if (bytes_received == 0) {
...
int main() {
    ...
    while (1) {
        read_fds = master_fds; // Copy the master set to the read set
        ...
        for (int i = 0; i <= max_fd; i++) {
            if (FD_ISSET(i, &read_fds)) {
                if (i == server_socket_fd) {
                    ...
                    FD_SET(client_socket_fd, &master_fds); // Add the new client to the master set
                    if (client_socket_fd > max_fd) {
                        max_fd = client_socket_fd; // Update the max_fd
                    }
                    ...
                    if (handle_client(i) == 0) {
                        // printf("Closing %d, max_fd: %d\n",i,max_fd);
                        close(i);
                        FD_CLR(i, &master_fds); // Remove the client from the master set
                    }
                }
            }
        }
